...
## Idea for "optimal paralleling"
#NUMCPUS?=$(shell nproc)
NUMCPUS?=$(shell grep "core id" /proc/cpuinfo  | uniq | wc -l)
MAKEFLAGS += --jobs --load-average=$(NUMCPUS)
...
MAKEFLAGS += --always-make --max-load=3 --output-sync
#--debug=jobs
...
pip:
	sudo pip_upgrade_outdated --pip_cmd "/usr/bin/python3 -m pip"  --quiet

apt:
	sudo apt-get --quiet --quiet update
	sudo apt-get --quiet --quiet -y dist-upgrade
...
inxiprep:
	sudo apt-get install -y ipmitool lvm2 mdadm smartmontools upower \
...
pythonprepext:
	sudo python3 -m pip install --upgrade vaex dask mypy-extensions nbformat widgetsnbextension jupyterlab-widgets ipydatawidgets ipywidgets
